...
from OpportunityHub.jobs.models import Job, Applicant
...
class CandidateOwnerRequiredMixin(AccessMixin):
    """Verify that the current user has this profile."""

    def dispatch(self, request, *args, **kwargs):
        if request.user.candidate.pk != kwargs.get('pk', None):
...
class JobEditMixin(AccessMixin):
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.user.pk == self.get_object().candidate.pk:
            return super().dispatch(request, *args, **kwargs)
        else:
...
#
#     def dispatch(self, request, *args, **kwargs):
#         if not self.request.user.candidate.activated:
#
#             messages.warning(request, "Please complete your company profile information before posting a job. Go to: Edit Company")
#             return redirect(reverse_lazy('company-dashboard'))
#         return super().dispatch(request, *args, **kwargs)
